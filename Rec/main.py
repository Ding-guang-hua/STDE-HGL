...
class Coach:
    ...
    def trainEpoch(self):
        ...
        epLoss, epRecLoss, eDiffLoss, eSSLoss, epclLoss = 0, 0, 0 ,0, 0
        ...
        for i, tem in enumerate(trnLoader):
            ...
            negs = negs.long().cuda()
            self.opt.zero_grad()
            loss, bprLoss, regLoss, diff_loss, clLoss = self.model.cal_loss(ancs,poss,negs)
            ...
            loss.backward()
            self.opt.step()
            log('Step %d/%d: loss = %.3f, diffLoss = %.3f,regLoss = %.3f,clLoss = %.3f' % (i, steps, loss, diff_loss , regLoss, clLoss), save=False, oneline=True)
