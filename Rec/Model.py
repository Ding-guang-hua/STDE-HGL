...
class HGDM(nn.Module):
    def __init__(self,data_handler):
        ...
        self.husr_denoiser = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm).to(device)
        self.hitem_denoiser = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm).to(device)
    ...
    def cal_loss(self,ancs,poss,negs):
        ...
        #buy
        u_diff_loss,diff_usrEmbeds= self.diffusion_process.target_training_losses(self.usr_denoiser, usrEmbeds, ancs)
        i_diff_loss,diff_itemEmbeds = self.diffusion_process.target_training_losses(self.item_denoiser, itmEmbeds, poss)
        diff_loss = (u_diff_loss.mean()+i_diff_loss.mean())

        #all
        hu_diff_loss, hdiff_usrEmbeds = self.diffusion_process.training_losses2(self.husr_denoiser, usrEmbeds,h_usrEmbeds, ancs)
        hi_diff_loss,hdiff_itemEmbeds = self.diffusion_process.training_losses2(self.hitem_denoiser, itmEmbeds, h_itemEmbeds, poss)
        hdiff_usrEmbeds = usrEmbeds+hdiff_usrEmbeds
        hdiff_itemEmbeds = itmEmbeds+hdiff_itemEmbeds
        h_diff_loss = (hu_diff_loss.mean() + hi_diff_loss.mean())

        diff_loss = diff_loss + h_diff_loss

        ancEmbeds = h_usrEmbeds[ancs]
        posEmbeds = h_itemEmbeds[poss]
        negEmbeds = h_itemEmbeds[negs]
        
            
        scoreDiff = pairPredict(ancEmbeds, posEmbeds, negEmbeds)
        bprLoss = - (scoreDiff).sigmoid().log().sum() / args.batch 
        regLoss = ((torch.norm(ancEmbeds) ** 2 + torch.norm(posEmbeds) ** 2 + torch.norm(negEmbeds) ** 2) * args.reg)/args.batch
        clLoss1 = (contrastLoss(h_usrEmbeds, diff_usrEmbeds, ancs, args.temp) + contrastLoss(h_itemEmbeds, diff_itemEmbeds, poss, args.temp)) * args.ssl_reg
        clLoss2 = (contrastLoss(h_usrEmbeds, hdiff_usrEmbeds, ancs, args.temp) + contrastLoss(h_itemEmbeds, hdiff_itemEmbeds, poss,args.temp)) * args.ssl_reg
        clLoss = clLoss1 + clLoss2

        loss = bprLoss + regLoss + diff_loss +clLoss
        return loss,bprLoss,regLoss,diff_loss,clLoss


    def predict(self):
        usrEmbeds, itmEmbeds, h_usrEmbeds, h_itemEmbeds = self.forward()
        denoised_u = self.diffusion_process.p_sample(self.usr_denoiser, usrEmbeds, args.sampling_steps)
        denoised_i = self.diffusion_process.p_sample(self.item_denoiser, itmEmbeds, args.sampling_steps)
        # usrEmbeds = usrEmbeds+denoised_u
        # itmEmbeds = itmEmbeds+denoised_i
        return h_usrEmbeds,h_itemEmbeds
...
class GaussianDiffusion(nn.Module):
    ...
    def target_training_losses(self, model, x_start, batch):
        batch_size = x_start.size(0)
        device = x_start.device
