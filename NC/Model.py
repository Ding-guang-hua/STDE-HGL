...
class HGDM(nn.Module):
    def __init__(self,f_dim):
        ...
        self.all_user_denoise_model = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm)
    ...
    def cal_loss(self, ancs, label,he_adjs,initial_feature):
        ...
        # main
        source_embeddings = source_embeddings1+source_embeddings2 + target_embedding
        # view1
        target_diff_loss, target_diff_embeddings = self.diffusion_model.target_training_losses(self.user_denoise_model, target_embedding, ancs)
        # view2
        diff_loss,diff_embeddings= self.diffusion_model.training_losses2(self.all_user_denoise_model, target_embedding, source_embeddings, ancs)
        diff_loss = (diff_loss.mean() + target_diff_loss.mean())
        all_embeddings = target_embedding+diff_embeddings

        scores = self.dense(source_embeddings)
        ...
        clLoss1 = contrastLoss(source_embeddings, target_diff_embeddings, ancs, args.temp) * args.ssl_reg
        clLoss2 = contrastLoss(source_embeddings, all_embeddings, ancs, args.temp) * args.ssl_reg
        clLoss = clLoss1 + clLoss2
        return nll_loss,diff_loss,clLoss
    ...
    def get_allembeds(self, he_adjs,initial_feature):
        ...
        scores = self.dense(source_embeddings)
        return source_embeddings,scores
